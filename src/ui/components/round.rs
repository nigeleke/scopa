...
#[component]
pub fn RoundEditor(game: Game<Playing>, round: Signal<Round>) -> Element {
    let teams = Teams::from(game.teams().clone());

    let first_active_team = teams
        .iter()
        .find(|t| t.is_playing())
        .expect("active team in playing game");
    let first_active_team_id = first_active_team.id();

    let leading_team_score = teams
        .iter()
        .map(|team| game.points(team.id()).expect("valid points"))
        .max()
        .unwrap_or_default();
    ...
    let team_column_components = move |team: &Team| {
        let id = team.id();
        let name = team.name();
        let points = game.points(id).expect("valid points");
...
fn RadioTeamIcon(
    ...
    let update_draft = move |_| {
        let new_round = match group {
            ...
            PointsGroup::Settebello => id.map_or_else(&*round, |id| round().with_settebello(id)),
